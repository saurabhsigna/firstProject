...
export default function App({ content }) {
  ...
  let size = "w-[90%] sm:w-[340px] max-w-[340px] flex flex-col h-auto ";
  ...
  return (
    <div className={`${size}`}>
      ...
      <div className="text-yellow-400 mt-[10px] uppercase">
        {content?.subject}
      </div>
      ...
      <button className="bg-[#5553eb] w-fit  rounded-full px-5 py-2">
        <img />
        <button onClick={clickHandler} className="text-lg text-white">
          View More
        </button>
      </button>
    </div>
