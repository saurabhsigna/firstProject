...
export default function SimpleAccordion({
  ...
  let chapterId = getChapterId(courseContent, currentVideoId);
        ...
        {courseContent.map((chapterContent, index) => (
          <Accordion
            defaultExpanded={
              ...
              chapterId ? chapterId == chapterContent.id : 6
            }
            key={index}
          >
            <AccordionSummary
            ...
            >
              <Typography>
                {" "}
                {chapterContent.id}. {chapterId ? chapterId : "no"}
              </Typography>
              <Typography>
                {" "}
            ...
            <AccordionDetails>
              {chapterContent?.chapterContent.map((videos, index) => (
                ...
                >
                  {videos?.isFree ? (
                    <Link href={`/testing/${courseId}/learn/${videos?.id}`}>
